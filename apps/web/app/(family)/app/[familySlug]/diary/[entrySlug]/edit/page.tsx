...
import { deleteDiaryAction, updateDiaryAction } from "src/actions/content-actions";
...
import { getDiaryRecord } from "src/lib/content-store";
...
export default async function EditDiaryPage(props: EditDiaryPageProps) {
  ...
  return (
    <FamilyAppShell
      actions={
        <div className="inline-actions">
          <Link className="button button--secondary" href={buildFamilyModuleDetailHref(familySlug, "diary", record.slug)}>
            상세로
          </Link>
      ...
      canManage={canManage}
      subtitle="수정 후 저장하면 일기 상세와 recent 흐름이 함께 갱신됩니다."
      title="일기 수정"
    ...
    >
      ...
      <SurfaceCard
        ...
        footer={
          <div className="inline-actions">
            <button className="button button--primary" form="diary-edit-form" type="submit">
              수정 저장
            </button>
            <Link className="button button--secondary" href={buildFamilyModuleDetailHref(familySlug, "diary", record.slug)}>
              취소
      ...
      >
        <form action={updateDiaryAction} className="form-stack" id="diary-edit-form">
          <input name="familySlug" type="hidden" value={familySlug} />
          <input name="currentSlug" type="hidden" value={record.slug} />
          <DiaryFormFields initial={record} />
        </form>
      </SurfaceCard>

      <SurfaceCard title="삭제" description="현재 일기를 제거하고 목록으로 돌아갑니다." tone="warm">
