...
export default async function FamilyBuilderPage(props: FamilyBuilderPageProps) {
  ...
  return (
    <PageShell
      ...
      subtitle="가족별 모듈 조합과 순서, 홈 프리셋, 입장 흐름을 서버에 저장하는 편집 화면입니다."
      actions={
    ...
    >
      <HeroCard
        eyebrow={workspaceView.family.source === "custom" ? "Custom Mini Home" : "Demo Base"}
        title={`${workspaceView.family.name} 홈과 입장을 조립하는 화면`}
        subtitle={`${getFamilyWorkspaceSummary(workspaceView)} 순서로 구성되어 있습니다. 저장하면 서버 파일에 기록되어 내부망에서 그대로 공유됩니다.`}
        meta={
          <>
      ...
      <div className="grid-two builder-summary-grid">
        <SurfaceCard
          title="지금 켜진 모듈"
          description="이 순서대로 가족 홈 카드와 입구 안내가 정렬됩니다."
          badge={<StatusPill>{workspaceView.workspace.enabledModules.length} modules</StatusPill>}
        >
        ...
        <SurfaceCard
          title="접속 정보"
          description="이 가족 홈은 내부망에서 아래 주소로 바로 열 수 있습니다."
